 ...
 * @returns {Array} [{slot_start, slot_end, remaining_capacity}]
 */
const generateAvailableSlots = async (service, date) => {
  const slots = [];
  const startMin = timeToMinutes(service.start_time || '09:00');
  const endMin = timeToMinutes(service.end_time || '18:00');
